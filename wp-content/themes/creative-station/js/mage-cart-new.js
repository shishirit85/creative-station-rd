...
mageCart = {
    ...
    selectors: {
        ...
        cartFormMobile: '#add-to-cart-mobile',
    ...
    init: function (customerSectionUrl, productsLoadUrl, skuString, requestKey) {
        ...
        this.customerSectionLoad().done(function () {
            if (this.skus.length > 0) {
                this.productDataLoad().done(function (response) {
    ...
    bindEvents: function () {
        ...
        $(this.selectors.cartFormMobile).on('submit', this.submit.bind(this));
    ...
    renderProductDataMobile: function () {
        ...
        _.each(this.productData, function (product) {
            ...
            if (product.status === '1') {
                ...
                productContainer.find('.price').html(product.price);
                productContainer.find('.special-price').html(product.special_price);
                productContainer.find('.thumbnail img').attr('data-src', product.thumbnail);
                if (!product.special_price) {
                    productContainer.find('.special-price').remove();
                } else {
                    productContainer.find('.price').addClass('was');
    ...
    submit: function (event) {
        ...
        $.ajax({
            ...
            data: formRequest,
            statusCode:{
                502: this.submissionTimeout.bind(this),
                404: this.submissionFail.bind(this),
                503: this.submissionFail.bind(this),
            }
        })
    ...
    submissionDone: function (response) {
        console.log(Storages.cookieStorage.get('mage-messages'), 'text');
        this.customerSectionLoad().done(function () {
    ...
    submissionTimeout: function(response){
        this.customerSectionLoad().done(function () {
            this.initAddToCartPopup();
            this.addedSkus = [];
            this.toggleLoader();
    ...
    initAddToCartPopup: function () {
        ...
        _.each(this.addedSkus, function (sku) {
            var productContainer = $('input[value=' + sku + ']').parents('.product.desktop');
    ...
    calculateTotalsMobile: function () {
        ...
        $(this.selectors.cartFormMobile + ' ' + this.selectors.productContainers + ':not(.unavailable)' + ':not(.desktop)').each(function (key, value) {
            console.log(value);
            var priceSelector = this.selectors.price;
